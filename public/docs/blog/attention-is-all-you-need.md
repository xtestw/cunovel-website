...
在 **Decoder** 里，生成第 *t* 个词时只能看到前面已生成的部分，不能看到未来，因此用 **Mask** 把「未来位置」在 softmax 前的 logits 置为极小值（常见实现写 `-1e9` 或负无穷），softmax 后对应权重约为 0，这就是 **因果掩码（Causal Mask）**。
...
**Encoder** 由多层（论文主实验里 *N* = 6）堆叠，每层大致包含：
...
去掉 RNN 以后，若**只有**词嵌入与自注意力、**没有**额外位置信息，则网络对「第几个位置」是对称的：无法仅凭结构区分谁先谁后，必须显式注入顺序信号。论文在词向量上 **加上** 固定的 **正弦 / 余弦位置编码**，让不同位置、不同维度上有不同相位与波长，从而可区分次序；论文也讨论了相对位置与长度外推等动机。后续很多模型改用可学习位置嵌入或其它编码，但「顺序要单独告诉模型」这一点仍然成立。
