...
const PhoneVerify = () => {
  ...
  const getInitialVerifyType = () => {
    if (pathname === '/phone-verify/online-time') {
        ...
        if (orderIds.length > 0 && isMounted) {
          ...
          try {
            ...
            if (paymentResult.status === 'paid') {
              // 如果已支付，跳转到订单详情页面
              router.push(`/verify-result?orderId=${latestOrderId}`);
      ...
      paymentCheckIntervalRef.current = setInterval(async () => {
        try {
          ...
          if (paymentResult.status === 'paid') {
            ...
            router.push(`/verify-result?orderId=${orderIdToCheck}`);
          ...
          <div className="type-nav-items">
            ...
            <button
              ...
              onClick={() => {
                ...
                handleReset();
                router.push('/phone-verify?type=mobile3Meta');
              }}
            >
            ...
            <button
              ...
              onClick={() => {
                ...
                handleReset();
                router.push('/phone-verify/online-time');
              }}
            >
