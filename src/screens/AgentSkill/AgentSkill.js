...
const AgentSkill = () => {
  ...
  /** 与 App `/agent-skill/*` 一致；尾部无 slug（如 `/agent-skill/`）时视为 intro */
  const routeDocPath = (() => {
    const m = pathname.match(/\/agent-skill\/(.*)/);
    const raw = m?.[1];
    return raw && raw.length > 0 ? raw : 'intro';
  })();
  const [sidebarData, setSidebarData] = useState([]);
  const [content, setContent] = useState('');
  /** 构建预渲染的 HTML 片段（优先于 Markdown 客户端渲染） */
  ...
  const loadDocument = async (path) => {
    ...
    try {
      ...
      const htmlTry = [];
      const pushHtml = (u) => {
        if (!htmlTry.includes(u)) htmlTry.push(u);
      };
      if (path === 'intro') {
        pushHtml(`${basePath}/home.html`);
      }
      pushHtml(`${basePath}/${actualPath}.html`);

      for (const htmlUrl of htmlTry) {
  ...
  // 获取当前页面路径
  const actualPath = routeDocPath === 'intro' ? 'home' : routeDocPath;
        ...
        <div className="agent-skill-main">
          ...
          {loading ? (
            ...
            <div className="error">
              ...
              <p>{error}</p>
              <button onClick={() => router.push('/agent-skill/intro')} className="back-button">
            ...
            <div className="markdown-content">
              <ReactMarkdown
                ...
                components={{
                  img: ({ node, ...props }) => {
                    let src = props.src || '';
                    const fileLogicalPath = routeDocPath === 'intro' ? 'home' : routeDocPath;
                    if (src && !src.startsWith('http') && !src.startsWith('/')) {
                      const pathParts = fileLogicalPath.split('/');
                      pathParts.pop();
