...
const OpenClawTutorial = () => {
  ...
  const routeDocPath = (() => {
    const suffix = pathname.replace(/^\/openclaw-tutorial\/?/, '').replace(/\/$/, '');
    return suffix || 'intro';
  })();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await fetch(`/docs/openclaw-tutorial/${lang}/navigation.json`);
  ...
  const loadDocument = useCallback(
    async (slug) => {
      ...
      try {
        ...
        const mdUrl = `/docs/openclaw-tutorial/${lang}/${encodePublicDocSegments(file)}`;
        const htmlFile = file.replace(/\.md$/i, '.html');
        const htmlUrl = `/docs/openclaw-tutorial/${lang}/${encodePublicDocSegments(htmlFile)}`;
  ...
  const handleNavClick = (path) => router.push(`/openclaw-tutorial/${path}`);
  ...
  const pageTitle =
    slugToTitle.get(routeDocPath) || slugToTitle.get('intro') || (isZh ? 'OpenClaw 源码教程' : 'OpenClaw Source Tutorial');
  const seoTitle = isZh
    ? `${pageTitle} - OpenClaw 源码教程 | CUTool`
    : `${pageTitle} - OpenClaw Source Tutorial | CUTool`;
  const seoDescription = isZh
    ? 'OpenClaw 开源 AI 助手框架源码分析教材：架构、流程、插件与 Skill 体系。'
    : 'OpenClaw open-source AI assistant framework: architecture, flows, plugins, and skills.';
  const seoKeywords = isZh
    ? 'OpenClaw,源码教程,AI Agent,插件,Skill'
    : 'OpenClaw,source code tutorial,AI agent,plugins,skills';

  const currentUrl =
    typeof window !== 'undefined'
      ? `${window.location.origin}${pathname}`
      : 'https://cutool.online/openclaw-tutorial/intro';
  ...
  const resolveImgSrc = (src) => {
    ...
    return `/docs/openclaw-tutorial/${lang}/${encodePublicDocSegments(base)}`;
  };
      ...
      <div className="agent-skill-container">
        <div className={`agent-skill-sidebar ${sidebarOpen ? 'open' : 'closed'}`}>
          <div className="sidebar-header">
            <h2>{t('common.openclawTutorial')}</h2>
            <button
              type="button"
        ...
        <div className="agent-skill-main">
          ...
          {!navReady || loading ? (
            ...
            <div className="error">
              ...
              <p>{error}</p>
              <button type="button" onClick={() => router.push('/openclaw-tutorial/intro')} className="back-button">
            ...
            <div className="markdown-content">
              <ReactMarkdown
                ...
                components={{
                  ...
                  a: ({ node, ...props }) => {
                    if (props.href && props.href.startsWith('/openclaw-tutorial/')) {
                      return (
                        <a
                          {...props}
                          href={props.href}
